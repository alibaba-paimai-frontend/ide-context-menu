...
router.get('getAllItems', '/items', function(ctx: IContext) {
  ...
  buildNormalResponse(ctx, 200, {
    items: meunItems.map((node: any) => pick(node, filterArray))
  });
});
...
router.get('getItemById', '/items/:id', function(ctx: IContext) {
  ...
  const filterArray = query && query.filter && query.filter.trim().split(',');
  buildNormalResponse(ctx, 200, {
    item: stores.model.menu.findItem(id, filterArray)
  });
});
 ...
 * 返回菜单的尺寸信息
 */
router.get('getCSize', '/csize', function(ctx: IContext) {
  const { stores } = ctx;
  buildNormalResponse(ctx, 200, {
    width: stores.model.cWidth,
    height: stores.model.cHeight
  });
});
